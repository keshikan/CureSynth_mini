...
void wmSetGPIO(bool logic)
{
	...
	wmWriteByte(WM_REG_ADDITIONAL_CONTROL_4, ret);
...
//return error state (0 is ok, 1 is error)
bool wmInit_process1()
{
	uint32_t errflag = 0x00;
...
//In order to prevent click noise, DAC need 1000 ms wait after WM_REG_MONO_OUT_MIX_1 and WM_REG_MONO_OUT_MIX_2.
//So I divided wmInit() into two functions, and insert long-time process between process1 and process2
bool wmInit_process2()
{
	...
	errflag |= wmWriteByte(WM_REG_ADDITIONAL_CONTROL_1, 0b111000001);
	...
	//Enable ADCLRC/GPIO1 Pin to Output
	errflag |= wmWriteByte(WM_REG_AUDIO_INTERFACE_2, 0b001000000);
